...
for i = 1:subj
    ...
    Right = smoothdata(right,'SmoothingFactor',0.05);
    Left = smoothdata(left,'SmoothingFactor',0.05);
 
    fig = figure('Name','1','units','normalized','outerposition',[0 0 1 1]);
    hold on;
    grid on;
    plot(1:len,Right(:,1),'b','Linewidth',2);
    plot(1:len,Right(:,2),'g','Linewidth',2);
    plot(1:len,Right(:,3),'c','Linewidth',2);
    plot(1:len,Left(:,1),'b');
    plot(1:len,Left(:,2),'g');
    plot(1:len,Left(:,3),'c');
    axis tight
